        ...
        for message in messages_list:
            try:
                ...
                if message_response.status_code != 200:
                    logger.warning(
                        f"Error fetching message {message_id}: {message_response.status_code}"
                    )
                    continue
                ...
                if "Realizaste un consumo" in cleaned_text:
                    ...
                    self.find_beneficiary(
                        cleaned_text, dict_to_send, BENEFICIARY_PATTERN
                    )
...
def _find_date(
    self: BcpEmailStrategy, cleaned_text: str, dict_to_send: dict[str, float | str]
):
